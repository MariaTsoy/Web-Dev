...
export function deleteAlbumById(id: number): void {
  albumsList = albumsList.filter(album => album.id !== id);
}

export function updateAlbumNameById(albumId: number, newName: string): void {
...
export function getAlbumById(albumId: number): any {
  return albumsList.find(album => album.id === albumId);
}
